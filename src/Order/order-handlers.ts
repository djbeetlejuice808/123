...
import { OrderStatus, PrismaClient, Prisma , PaymentMethodType, PaymentStatus } from '@prisma/client';
...
const createOrder = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const orderCode = signatureUtils.uuid();
        ...
        if (!paymentConfigFromServer) {
            ...
            if (paymentConfigFromServer.type === PaymentMethodType.BANK_TRANSFER) {
                ...
                const bankTransferPayment = await prisma.payment.create({
                    data: {
                        orderId: orderFromServer.id,
                        amount: orderFromServer.totalAmount,
                ...
                let paymentMethodConfig = payment?.paymentMethod?.config as Record<string, any>;
                let baseUrl = paymentMethodConfig?.baseUrl;
                const now = Math.floor(Date.now() / 1000);
                const validExpiredAt = now + 3600;
                let payosData: CheckoutRequestType = {
                    orderCode: orderFromServer.id,
                    amount: Number(orderFromServer.totalAmount),
                    description: signatureUtils.generateRandomNumber(10).toString(),
                    cancelUrl: `${baseUrl}/api/cancel/`,
                    returnUrl: `${baseUrl}/`,
                    expiredAt: validExpiredAt
                };

                payosData.signature = signatureUtils.createSignature({
                    orderCode: orderFromServer.id,
                    amount: Number(orderFromServer.totalAmount),
                    description: signatureUtils.generateRandomNumber(10).toString(),
                    cancelUrl: `${baseUrl}/api/cancel/`,
                ...
                const payosClient = new PayOS(paymentMethodConfig.clientId,
                   paymentMethodConfig.apiKey, paymentMethodConfig.checkSum
                  );

                const response = await payosClient.createPaymentLink(payosData);
