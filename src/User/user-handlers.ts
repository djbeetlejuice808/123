...
const getWards = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Lỗi khi lấy danh sách phư��ng/xã' });
    }
};

const updateUser = async (req: Request, res: Response) => {
    const { id } = req.params;
    const { fullName, avatar, status, type } = req.body;
    ...
    try {
        ...
        const userFromServer = await prismaService.user.update({
            where: { id: Number(id) },
            data: { 
                ...(fullName && { fullName }),
                ...(avatar && { avatar }),
